...
function elapsed(createdAt: string): string {
  const started = Date.parse(createdAt); return Number.isNaN(started) ? "N/A" : `${Math.max(0, (Date.now() - started) / 1000).toFixed(1)}s`;
}

export function JobsTable() {
  const { jobs, selectedJobId, detail, cancellationPending, selectJob, cancelJob, refreshJobs } = useJobs();
  return (
      ...
      <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead className="text-xs uppercase tracking-wider text-slate-500"><tr className="border-b border-slate-800"><th className="px-4 py-2">Job ID</th><th className="px-4 py-2">Task Type</th><th className="px-4 py-2">Status</th><th className="px-4 py-2">Duration</th><th className="px-4 py-2">Created At</th><th className="px-4 py-2 text-right">Actions</th></tr></thead>
        <tbody>{jobs.map((job) => {
          ...
          return <tr key={job.job_id} onClick={() => selectJob(job.job_id)} className={`cursor-pointer border-b border-slate-800/60 hover:bg-slate-800/50 ${selectedJobId === job.job_id ? "bg-slate-800" : ""}`}>
            <td className="max-w-64 truncate px-4 py-2 font-mono text-xs text-cyan-300" title={job.job_id}>{job.job_id}</td><td className="px-4 py-2 text-xs font-bold uppercase text-slate-300">{job.task_type}</td><td className="px-4 py-2"><StatusBadge status={job.status} /></td><td className="px-4 py-2 font-mono text-xs">{active ? elapsed(job.created_at) : selectedJobId === job.job_id && detail?.job_id === job.job_id ? detail.duration || "—" : "—"}</td><td className="px-4 py-2 text-xs text-slate-400">{new Date(job.created_at).toLocaleTimeString()}</td>
